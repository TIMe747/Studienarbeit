...
\begin{framed}
	\begin{algorithm}[H]
		...
		\While{Maximale Iterationen nicht erreicht}{
			...
			Bestimme für jeden Partikel Geschwindigkeit 
...
\begin{itemize}
	...
	\item Wahrscheinlichkeiten $W=\{w_{n}(t),w_{no}(t),w_{o}(t),w_{os}(t),w_{s}(t),w_{sw}(t),w_{w}(t),w_{wn}(t)\}$  zur Wahl von $x_{i}(t)$: 	
...
\begin{framed}
	\begin{algorithm}[H]
		...
		\While{Maximale Iterationen nicht erreicht}{
			Berechne für jede modellierte Ameise die Wahrscheinlichkeiten $W$ für die möglichen Teillösungen $Z$ ausgehend von Position $x_{i}(t-1)$\;
			Bestimme für jede modellierte Ameise neue Position $x_{i}(t+1)$\;
			\lIf{$f(x_{i}(t))$ besser als $f(x_{g}(t))$}{
				Setze $x_{g}(t))$ = $x_{i}(t)$
			}
			Berechne für jede gewählte Teillösung aus $Z$ die neue Pheromonmarkierung $\tau_{i}(t+1)$ für Position $P$ im Suchraum $S$\;
		}
...
\end{equation}
Die Wahrscheinlichkeit ist abhängig von dem Pheromonniveau $tau_{C}$ der jeweiligen Teillösung und der Güte der Teillösung bestimmt durch $f(C(t))$. Die Parameter $\alpha$ und $\beta$ verändern das Verhalten der modellierten Ameisen. Gilt $\alpha > \beta$ dann ist das Pheromonniveau der Teillösung ausschlaggebender als die Güte der Teillösung für die Berechnung der Wahrscheinlichkeit $W$. Sind die Wahrscheinlichkeiten $W$ für alle Teillösungen $C$ berechnet, entscheidet sich die modellierte Ameise für die Teillösung mit der höchsten Wahrscheinlichkeit. Die von der modellierten Ameise gewählte Teillösung aus $C$ entspricht der neuen Position $x_{i}(t)$. Im Anschluss wird für jede modellierte Ameise $x_{i}(t)$ geprüft, ob der aktuelle Funktionswert $f(x_{i}(t))$  besser ist als der globale Funktionswert $f(x_{g}(t))$. Ist dies der Fall so wird die global beste Position $x_{g}(t)$ zu $x_{i}(t)$. Am Ende der Iteration werden die Pheromonmarkierungen mit folgender Formel aktualisiert:
\begin{equation}
\tau_{i}(t+1) = (1 - \rho) \tau_{i}(t) + \sum_{x_{i} \in X}\ \Delta \tau_{i}(t)^C
\end{equation}
Hierbei ist $\rho$ der konstante Verdunstungsfaktor der Pheromonmarkierungen. Die an der Teillösung hinterlassene Pheromonmenge ist abhängig von $\Delta \tau_{i}(t)^C$ und berechnet sich durch die Formel:
\begin{equation}
\Delta \tau_{i}(t)^C = f(C(t)
\end{equation}
\newpage
