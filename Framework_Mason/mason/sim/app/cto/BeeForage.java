...
public /*strictfp*/ class BeeForage extends SimState
    {
    ...
    /** Creates a BeeForage simulation with the given random number seed. */
    public BeeForage(long seed)
        {
        super(seed);
        }
        ...
        if( misteriousObjects != null )
            ...
            for( int i = 0 ; i < misteriousObjects.numObjs ; i++ )
                ...
                if( misteriousObjects.objs[i] != null && misteriousObjects.objs[i] != agent )
                    {
                    Object ta = (Bee)(misteriousObjects.objs[i]);
    ...
    public void start()
        ...
        for(int x=0;x<NUM_AGENTS+NUM_TARGETS;x++)
            ...
            do
                ...
                if( x < NUM_AGENTS )
                    {
                    agent = new Bee( loc, Bee.AGENT, "Agent"+x );
                    }
                else
                    {
                    agent = new Bee( loc, Bee.TARGET, "Target"+(x-NUM_AGENTS) );
                    }
                times++;
                if( times == 1000 )
                    {
