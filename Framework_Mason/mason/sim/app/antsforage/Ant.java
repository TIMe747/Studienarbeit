...
public class Ant extends OvalPortrayal2D implements Steppable {
	    ...
	    public void depositPheromone( final SimState state) {
	        ...
	        if (AntsForage.ALGORITHM == AntsForage.ALGORITHM_VALUE_ITERATION) {
	        	...
	        	/*ant follow home pheromones and ant drop food pheromones*/
	        	if (hasFoodItem) { 
		            ...
		            for(int dx = -1; dx < 2; dx++)
		            	for(int dy = -1; dy < 2; dy++) {
			                ...
			                double m = af.toFoodGrid.field[_x][_y] * (dx * dy != 0 ? af.diagonalCutDown : af.updateCutDown) + reward;
			                if (m > max) {
			                	max = m;
			                }
		                }
			         ...
			         /*ant follow food pheromones and ant drop home pheromones*/
			                ...
			                for(int dx = -1; dx < 2; dx++)
			                    for(int dy = -1; dy < 2; dy++){
				                    ...
				                    double m = af.toHomeGrid.field[_x][_y] * (dx * dy != 0 ? af.diagonalCutDown : af.updateCutDown) + reward;
				                    if (m > max) {
				                    	max = m;
				                    }
			                     }
